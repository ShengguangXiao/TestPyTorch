...
const int PR_GROUP_TEXTURE_IMG_COUNT_4 =        4;
const int PR_GROUP_TEXTURE_IMG_COUNT_3 =        3;
const int PR_MAX_AUTO_THRESHOLD_COUNT =         10;
const int DEFAULT_TABLE_MAPPING_BEZIER_RANK =   5;
const int NUM_OF_DLP =                          4;
const size_t GIGA_BYTE =                        1024 * 1024 * 1024;
...
enum class PR_RM_FIT_NOISE_METHOD {
    ...
    POSITIVE_ERR,   //Remove the point with positive error over the error tolerance. If it is circle, remove the standalone outer points of the fitted circle.
    NEGATIVE_ERR,   //Remove the point with negative error over the error tolerance. If it is circle, remove the standalone inner points of the fitted circle.
};
...
enum class BARCODE_TYPE {
    CODE_1D,
    CODE_2D,
};

enum class PR_FIND_BLOB_METHOD {
    CONTOUR,
    MIN_AREA,
};
