...
VisionAPI void         PR_UnzipLogCase(const std::string &strPath);
...
VisionAPI VisionStatus PR_LoadRecord(Int32 nRecordId);
...
VisionAPI VisionStatus PR_GetRecordInfo(const std::string& strPath, PR_GET_RECORD_INFO_RPY *const pstRpy);
VisionAPI VisionStatus PR_RoateRecord(const PR_ROTATE_RECORD_CMD *const pstCmd, PR_ROTATE_RECORD_RPY *const pstRpy);
VisionAPI int          PR_GetMaxRecordId();
...
VisionAPI VisionStatus PR_FindEllipse(const PR_FIND_ELLIPSE_CMD *const pstCmd, PR_FIND_ELLIPSE_RPY *const pstRpy);
...
VisionAPI VisionStatus PR_CalibrateCameraNew(const PR_CALIBRATE_CAMERA_NEW_CMD *const pstCmd, PR_CALIBRATE_CAMERA_NEW_RPY *const pstRpy);
...
VisionAPI VisionStatus PR_MotorCalib3DGpu(const PR_MOTOR_CALIB_3D_CMD *const pstCmd, PR_MOTOR_CALIB_3D_NEW_RPY *const pstRpy);

/** @brief After the machine running for some days, the 3D base may shift, so use this function to calculate the shift.
Example:
@code{.cpp}
    PR_ONLINE_CALIB_3D_CMD stCmd;
    ... // Initialize the input iamge
    ... // Initialize the DLP parameters from 3D base calib result
    PR_ONLINE_CALIB_3D_RPY stRpy;
    PR_OnlineCalib3D(stCmd, stRpy);
@endcode
@param pstCmd pointer of input command.
@param pstRpy pointer of reply.
*/
VisionAPI VisionStatus PR_OnlineCalib3D(const PR_ONLINE_CALIB_3D_CMD *const pstCmd, PR_ONLINE_CALIB_3D_RPY *const pstRpy);

VisionAPI VisionStatus PR_Rebuild3DBase(const PR_REBUILD_3D_BASE_CMD *const pstCmd, PR_REBUILD_3D_BASE_RPY *const pstRpy);
...
VisionAPI VisionStatus PR_RunMergeHeight(const PR_RUN_MERGE_HEIGHT_CMD *const pstCmd, PR_RUN_MERGE_HEIGHT_RPY *const pstRpy);
...
VisionAPI VisionStatus PR_CreateOcvRecord(const PR_OCV_RECORD_INFO *const pstRecordInfo, Int32& nRecordId);
VisionAPI VisionStatus PR_Ocv(const PR_OCV_CMD *const pstCmd, PR_OCV_RPY *const pstRpy);
VisionAPI VisionStatus PR_ReadBarcode(const PR_READ_BARCODE_CMD *const pstCmd, PR_READ_BARCODE_RPY *const pstRpy);
...
VisionAPI VisionStatus PR_InspExtraUnit(const PR_INSP_EXTRA_UNIT_CMD* const pstCmd, PR_INSP_EXTRA_UNIT_RPY* const pstRpy);
VisionAPI VisionStatus PR_FindEmptyArea(const PR_FIND_EMPTY_AREA_CMD* const pstCmd, PR_FIND_EMPTY_AREA_RPY* const pstRpy);
VisionAPI VisionStatus PR_FindMark(const PR_FIND_MARK_CMD* const pstCmd, PR_FIND_MARK_RPY* const pstRpy);
VisionAPI VisionStatus PR_FindFocusArea(const PR_FIND_FOCUS_AREA_CMD* const pstCmd, PR_FIND_FOCUS_AREA_RPY* const pstRpy);
VisionAPI VisionStatus PR_BlendImg(const PR_BLEND_IMG_CMD* const pstCmd, PR_BLEND_IMG_RPY* const pstRpy);
VisionAPI VisionStatus PR_GenDeviceCad(const PR_GEN_DEVICE_CAD_CMD* const pstCmd, PR_GEN_DEVICE_CAD_RPY* const pstRpy);
VisionAPI VisionStatus PR_FindLargestBlob(const PR_FIND_LARGEST_BLOB_CMD* const pstCmd, PR_FIND_LARGEST_BLOB_RPY* const pstRpy);
VisionAPI VisionStatus PR_FindLeadCount(const PR_FIND_LEAD_COUNT_CMD* const pstCmd, PR_FIND_LEAD_COUNT_RPY* const pstRpy);
VisionAPI VisionStatus PR_GammaMergeImg(const PR_GAMMA_MERGE_IMG_CMD* const pstCmd, PR_GAMMA_MERGE_IMG_RPY* const pstRpy);
VisionAPI VisionStatus PR_GpuErode(const PR_GPU_ERODE_CMD* const pstCmd, PR_GPU_ERODE_RPY* const pstRpy);
VisionAPI VisionStatus PR_GpuDilate(const PR_GPU_DILATE_CMD* const pstCmd, PR_GPU_DILATE_RPY* const pstRpy);
VisionAPI VisionStatus PR_CalcShadowMask(const PR_CALC_SHADOW_MASK_CMD* const pstCmd, PR_CALC_SHADOW_MASK_RPY* const pstRpy);
VisionAPI VisionStatus PR_DeviceCorrection(const PR_DEVICE_CORRECTION_CMD* const pstCmd, PR_DEVICE_CORRECTION_RPY* const pstRpy);
